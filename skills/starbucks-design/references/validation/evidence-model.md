...
本文件定义 DesignKit Starter Capability 验证证据的统一记录格式。验证目标和通过标准读取 [Starter Validation Contract](validation-contract.md)；本模型只记录实际采用的方法、证据位置和结果，不把规则或资产存在误写为已验证。
...
Geometry / Composition Fidelity 记录还必须说明：同 viewport 的 Golden/generated computed geometry、relative relationships、delta、surface/typography、screenshot paths 和 responsive variables。

Implementation Provenance 记录还必须说明：Runtime resource loading、approved export、实际 DOM/component evidence、theme binding 和 native substitute 检查结果。

Visual Fidelity 记录还必须说明：Golden 与 generated screenshot 路径、viewport、theme、Reviewer status（`PASS` / `CONDITIONAL` / `FAIL` / `UNVERIFIED`）、审查日期和 Manual Review notes。没有人工签收时不得从 Geometry、Theme 或 Structural 结果推导 Visual `PASS`。

State Coverage 记录必须列出适用状态（例如 Normal、Loading、Empty、Error、Retry）及每个状态的实际 Browser 或 deterministic fixture evidence；Validation Fixture 足以证明状态路径时，不要求为每个状态创建 Golden variant。
...
本阶段只建立 Evidence Contract，不生成虚构的历史证据记录，也不修改 Runtime Manifest。
