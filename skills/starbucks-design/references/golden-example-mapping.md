...
| `starter.pattern.default-application-shell` | Starter | 以固定 Top + Side + Main outer composition 包裹已批准页面模板 | [Default Application Shell Contract](application-shell.md) Shell Mode Decision | 已选 Starter Template；Shell 只负责 outer wrapper | None；Docs Menu demos 状态为 `IMPLEMENTATION REFERENCE`，不是 Starter Golden | [Validation Contract](validation/validation-contract.md) Default Application Shell；Knowledge/projection contract test；test-only composition fixture；最终 ZIP package-only clean-room | `READY` | Support state `SUPPORTED`；无独立 Shell Golden，最终 Browser Evidence 记录在 Starter package smoke/report |
