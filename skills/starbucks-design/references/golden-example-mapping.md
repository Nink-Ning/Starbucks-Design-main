...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `starter.template.basic-list` | Starter | 扫描、对齐和比较高密度结构化数据 | [Template Selection](decisions/template-selection.md) Rule 1、Rule 3 | [Basic List Template](../../../distribution/designkit-starter-v1/templates/list.md) + [approved Basic List reference](../../../distribution/designkit-starter-v1/patterns/basic-list.html) | [Basic List Golden](../../../distribution/designkit-starter-v1/examples/list.html) | [Quality Checklist](../../../distribution/designkit-starter-v1/references/quality-checklist.md)、[Template Tests](../../../packages/docs/site/src/styles/__tests__/template-pages.test.mjs)、[Composition Fidelity](../../../packages/docs/site/src/styles/__tests__/starter-shell-template-composition-r2f.test.mjs) | `READY` | P1：Responsive、Theme、Accessibility 和 Interaction 的最近一次执行证据尚未按 Capability 独立登记 |
...
| `starter.pattern.default-application-shell` | Starter | 以固定 Top + Side + Main outer composition 包裹已批准页面模板 | [Default Application Shell Contract](application-shell.md) Shell Mode Decision | [approved Shell reference](../../../distribution/designkit-starter-v1/patterns/default-application-shell.html) + 已选 Starter Template；Shell 只负责 outer wrapper | None；reference asset 与 Docs Menu demos 都不是 Starter Golden | [Validation Contract](validation/validation-contract.md) Default Application Shell；Knowledge/projection contract test；test-only composition fixture；Composition Fidelity；最终 ZIP package-only clean-room | `READY` | Support state `SUPPORTED`；Shell reference status 为 `approved reference implementation`，无 Shell Golden |
...
- validation strategy：test-only composition fixture 验证批准实现；最终 ZIP 的 package-only clean-room 验证 1280/768/390、Light/Dark、overflow、ownership 和 accessibility。

Implementation reference 只证明应复用的批准模式；实际 Starter browser `PASS` evidence 必须来自 test-only fixture 或最终 ZIP clean-room。
