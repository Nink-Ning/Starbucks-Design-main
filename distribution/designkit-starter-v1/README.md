...
- 窄屏布局和表格局部横向滚动
- 三个已验证的 Golden Example：`examples/list.html`、`examples/form.html`、`examples/detail.html`
...
- `templates/`：列表、表单、详情三类页面的组合规则。
- `prompts/`：新建、修改和自检提示词。
- `assets/`：仅包含页面级布局样式，不包含组件内部样式。
- `examples/`：三个已验证的 Golden Example，分别对应基础列表、基础表单和基础详情。
