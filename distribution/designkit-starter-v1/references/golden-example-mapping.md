...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `starter.template.basic-list` | 扫描、对齐和比较高密度结构化数据 | [Template Selection](decisions/template-selection.md) Rule 1、Rule 3 | [Basic List Template](../templates/list.md) + [approved Basic List reference](../patterns/basic-list.html) | [Basic List Golden](../examples/list.html) | [Quality Checklist](quality-checklist.md)、[Validation Contract](validation/validation-contract.md) | `READY` | Reference asset 是模板实现基线，不是 Golden；Evidence Record 按执行结果维护 |
...
| `starter.pattern.default-application-shell` | 以固定 Top + Side + Main outer composition 包裹已批准 Template | [Application Shell Contract](application-shell.md) Shell Mode Decision | [approved Shell reference](../patterns/default-application-shell.html) + 已选 Starter Template；Shell 只负责 outer wrapper | None；Shell reference 与 Docs Menu patterns 都不是 Starter Golden | [Quality Checklist](quality-checklist.md)、[Validation Contract](validation-contract.md)、[Default Application Shell Evidence](validation/evidence/starter-pattern-default-application-shell.md) | `READY` | Support state `SUPPORTED`；Shell reference status 为 `approved reference implementation`，无独立 Shell Golden |
