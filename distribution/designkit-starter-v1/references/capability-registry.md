...
`starter.pattern.default-application-shell` 的产品 Support state 为 `SUPPORTED`；本 Registry 使用既有合法 `READY` 表示可按固定 Boundary 生成，不表示后续 Shell Browser Evidence 已完成。
