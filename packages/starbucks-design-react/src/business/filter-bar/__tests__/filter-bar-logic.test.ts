...
import {
  buildResetBaseline as buildVueResetBaseline,
  normalizeActiveSnapshot as normalizeVueActiveSnapshot,
} from '../../../../../starbucks-design-vue/src/business/filter-bar/normalize'
import {
  resolveFilterLayout as resolveVueFilterLayout,
  resolveResponsiveFieldSpans as resolveVueResponsiveFieldSpans,
} from '../../../../../starbucks-design-vue/src/business/filter-bar/layout'
import {
  validateFilterFields as validateVueFilterFields,
} from '../../../../../starbucks-design-vue/src/business/filter-bar/validation'
import type {
  FilterFieldSchema as VueFilterFieldSchema,
  FilterValue as VueFilterValue,
} from '../../../../../starbucks-design-vue/src/business/filter-bar/interface'
...
describe('FilterBar pure contract', () => {
  ...
  it('keeps React and Vue pure outputs equivalent for the same contract input', () => {
    const vueFields = fields as VueFilterFieldSchema[]
    const values = {
      keyword: 'coffee',
      status: 'open',
      tags: ['reserve'],
    ...
    expect(buildResetBaseline(fields, values)).toEqual(
      buildVueResetBaseline(vueFields, values as VueFilterValue),
    )
    expect(normalizeActiveSnapshot(fields, values)).toEqual(
      normalizeVueActiveSnapshot(vueFields, values as VueFilterValue),
    )
    expect(resolveFilterLayout(fields, { expanded: false, defaultVisibleCount: 2 })).toEqual(
      resolveVueFilterLayout(vueFields, { expanded: false, defaultVisibleCount: 2 }),
    )
    expect(resolveResponsiveFieldSpans(4, { xs: 1, sm: 1, md: 2, lg: 3, xl: 4, xxl: 4 })).toEqual(
      resolveVueResponsiveFieldSpans(4, { xs: 1, sm: 1, md: 2, lg: 3, xl: 4, xxl: 4 }),
    )
    expect(validateFilterFields(fields, values)).toEqual(
      validateVueFilterFields(vueFields, values as VueFilterValue),
    )
  })
})
