...
function findPrecedingHeading(lines: string[], fenceLineIdx: number): string | null {
  for (let i = fenceLineIdx - 1; i >= 0; i--) {
    ...
    if (m) {
      ...
      // Skip API section headings
      if (title.startsWith('<') || title === 'API' || title.startsWith('`')) continue
      return title
    }
  }
  return null
...
function migrateFile(mdPath: string): { demos: number; component: string } {
  ...
  while (i < lines.length) {
    ...
    if (line.trim().match(/^```vue\s+preview\s*$/)) {
      ...
      // Find preceding heading for title by searching backward through processed output
      const heading = findPrecedingHeading(resultParts, resultParts.length - 1)
      ...
      const demoCode = fenceContent.trim()
      const demoPath = join(demosDir, `${slug}.vue`)
      writeFileSync(demoPath, demoCode, 'utf-8')

      // Generate replacement
      const relPath = `./demos/${componentName}/${slug}.vue`
      resultParts.push(`<preview path="${relPath}" title="${title}"></preview>`)
      resultParts.push('')
  ...
  writeFileSync(mdPath, resultParts.join('\n'), 'utf-8')
  return { demos: demoCount, component: componentName }
}

function main() {
  const categories = readdirSync(COMPONENTS_DIR, { withFileTypes: true })
    .filter(d => d.isDirectory() && d.name !== 'demos')
    .map(d => d.name)
  ...
  for (const category of categories) {
    ...
    for (const mdFile of mdFiles) {
      ...
      console.log(`  Processing ${category}/${mdFile}...`)
      const { demos, component } = migrateFile(mdPath)
      if (demos > 0) {
        console.log(`  ✅ ${category}/${component}.md → ${demos} demos extracted`)
        total += demos
      }
    }
  }
