...
本文件维护 Starter Capability 从 Intent、Decision、Template 到 Golden Example 和 Validation 的包内可追踪关系。它只登记 `starter.*` Capability 和 `examples/` 中的只读 Golden，不修改 Template、Golden、Runtime 或组件实现。
...
| Golden Example | 包内只读参考；没有独立 Golden 时明确标记间接映射。 |
...
新增能力必须同步 Capability Registry、Decision Rule、Template、Golden Example 和 Validation。所有链接必须指向 Starter 包内资产；不得引入包外实现、测试或知识路径。

## 6. Remaining knowledge
