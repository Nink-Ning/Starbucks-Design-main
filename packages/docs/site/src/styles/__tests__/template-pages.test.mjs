...
test('card list template documents its component boundary and responsive rules', async () => {
  ...
  assert.match(doc, /仅在 Docs 实际使用时复用 TableToolbar/)
  assert.match(doc, /Card DOM、圆形 media、footer\/actions 和 selection treatment 直接复用 Docs Card List approved structure/)
  assert.match(doc, /对象操作和 More Menu 必须支持键盘操作/)
  assert.match(doc, /IconMore/)
  assert.match(doc, /图标按钮/)
  assert.match(doc, /Card Actions 与 Batch Actions 的目标边界/)
...
test('completed page templates document the shared header action rules', async () => {
  ...
  for (const source of sources) {
    assert.match(source, /左侧(?:有意义的)?面包屑或页面标题、右侧业务核心操作或全局功能操作|页面头部使用页面标题和可选的标题相邻 Context Help；depth ≤ 2 不显示 Back\/Breadcrumb|二级(?:全页|页面) (?:Create\/Edit|Grouped Form|Step Form) 使用(?:共享 Page Header 的)? icon-only Back|页面头部使用共享 Page ?Header/)
...
test('page template docs hide document chrome and use filled content area', async () => {
  ...
  assert.match(breadcrumb, /templates\/data-list\/basic-list[\s\S]*门店列表/)
  assert.match(breadcrumb, /'templates\/data-list\/card-list', \['card-list', '商品列表'/)
  assert.match(breadcrumb, /'templates\/data-list\/filter-list', \['filter-list', '门店列表'/)
  assert.match(breadcrumb, /'templates\/data-list\/tree-filter-list', \['tree-filter-list', '门店列表'/)
  assert.match(breadcrumb, /'templates\/data-list\/tag-list', \['tag-list', '标签管理'/)
  assert.match(breadcrumb, /\['basic-list', 'card-list', 'filter-list', 'tree-filter-list', 'tag-list'\]\.includes\(pageHeaderId\)/)
  ...
  assert.doesNotMatch(breadcrumb, /data-template-action-host="(?:basic-list|filter-list|tree-filter-list|tag-list)"/)
  ...
  assert.match(demoCss, /\.sl-container:has\(\.sb-template-page-breadcrumb\),[\s\S]*?\.sl-container:has\(\.sb-template-page-header\),[\s\S]*?\.sl-container:has\(\.sb-demo\[data-demo\^='template-'\]\)\s*\{[\s\S]*?max-width:\s*none;/)
  assert.match(demoCss, /\.main-pane:has\(\.sb-template-page-(?:breadcrumb|header)\)[\s\S]*?main > \.content-panel \+ \.content-panel\s*\{[\s\S]*?min-height:\s*calc\(100dvh - var\(--sb-docs-nav-height, 64px\)\);[\s\S]*?padding-inline:\s*20px;[\s\S]*?background:\s*var\(--bg-color-page\);/)
  assert.match(demoCss, /\.main-pane:has\(\.sb-template-page-(?:breadcrumb|header)\)[\s\S]*?main > \.content-panel \+ \.content-panel > \.sl-container\s*\{[\s\S]*?width:\s*100%;[\s\S]*?max-width:\s*none;[\s\S]*?min-height:\s*inherit;[\s\S]*?padding-inline:\s*0;/)
  assert.match(demoCss, /\.main-pane:has\(\.sb-template-page-(?:breadcrumb|header)\)[\s\S]*?background:\s*var\(--bg-color-page\);/)
  assert.match(demoCss, /\.main-pane:has\(\.sb-template-page-(?:breadcrumb|header)\)[\s\S]*?footer\.sl-flex\s*\{[\s\S]*?display:\s*none;/)
  ...
  assert.match(demoCss, /\.sb-template-page-header\s*\{[\s\S]*?width:\s*100%;[\s\S]*?margin:\s*0 0 16px;/)
  assert.match(demoCss, /\.sb-template-page-header \.sbux-pro-page-header\s*\{[\s\S]*?display:\s*flex;[\s\S]*?justify-content:\s*space-between;/)
  ...
  for (const source of [reactBasicList, vueBasicList]) {
    ...
    assert.doesNotMatch(source, /data-template-action-host="basic-list"|backable|backable=/)
  }
...
test('tag management template is a real React and Vue page composition', async () => {
  ...
  for (const source of [reactDemo, vueDemo]) {
    ...
    assert.match(source, /PageHeader/)
    assert.match(source, /helpText=|help-text=/)
    assert.match(source, /data-template-page-header-host="tag-list"/)
    assert.doesNotMatch(source, /data-template-action-host="tag-list"|backable|backable=/)
    ...
    assert.match(source, /标签组/)
    assert.match(source, /newTagGroupId|new-tag-group-id/)
    assert.match(source, /setActiveGroupId\(targetGroupId\)|activeGroupId\.value\s*=\s*targetGroupId/)
  ...
  assert.match(demoCss, /\.sb-tag-list-page__required-mark\s*\{[\s\S]*?color:\s*var\(--color-danger/)
...
test('tree filter list template keeps React and Vue query contracts equivalent', async () => {
  ...
  for (const source of [reactDemo, vueDemo]) {
    ...
    assert.match(source, /PageHeader/)
    assert.match(source, /helpText=|help-text=/)
    assert.match(source, /data-template-page-header-host="tree-filter-list"/)
    assert.doesNotMatch(source, /data-template-action-host="tree-filter-list"|backable|backable=/)
  ...
  assert.match(demoCss, /@media \(min-width:\s*1024px\)[\s\S]*?\.sb-tree-filter-list-page__layout\s*\{[\s\S]*?min-height:\s*inherit;/)
  ...
  assert.match(demoCss, /\.sb-tree-filter-list-page__table-module\s*\{[\s\S]*?display:\s*flex;[\s\S]*?flex:\s*1 1 auto;[\s\S]*?flex-direction:\s*column;/)
  assert.match(demoCss, /\.sb-tree-filter-list-page__pagination\s*\{[\s\S]*?margin-top:\s*auto;[\s\S]*?margin-inline:\s*calc\(-1 \* var\(--spacing-6\)\);[\s\S]*?margin-bottom:\s*calc\(-1 \* var\(--spacing-5\)\);[\s\S]*?padding:\s*var\(--spacing-6\) var\(--spacing-6\) var\(--spacing-5\);[\s\S]*?border-top:\s*1px solid var\(--color-border-1\);/)
  ...
  assert.match(demoCss, /\.sb-tree-filter-list-page > \.sb-tree-filter-list-page__layout > \.sb-tree-filter-list-page__content\s*\{[\s\S]*?display:\s*flex;[\s\S]*?flex-direction:\s*column;[\s\S]*?margin:\s*0;[\s\S]*?padding:\s*var\(--spacing-5\) var\(--spacing-6\);[\s\S]*?background:\s*var\(--bg-color-container\);[\s\S]*?border-radius:\s*0 var\(--border-radius-md\) var\(--border-radius-md\) 0;/)
...
test('basic form template keeps React and Vue form contracts equivalent', async () => {
  ...
  assert.match(sharedFormPageRule, /width:\s*100%;[\s\S]*?padding:\s*var\(--spacing-8\) var\(--spacing-10\) var\(--spacing-6\)[\s\S]*?font-family:\s*var\(--font-family\);[\s\S]*?background:\s*var\(--bg-color-container\)[\s\S]*?border-radius:\s*var\(--border-radius-md\)/)
...
test('filter list template keeps the current filter-enabled basic list composition', async () => {
  ...
  assert.match(doc, /筛选区卡片与顶部 PageHeader 保持 16px 间距/)
  assert.match(doc, /<code>TableToolbar<\/code>/)
  assert.doesNotMatch(doc, /TemplatePagePlaceholder/)
  ...
  for (const source of [reactDemo, vueDemo]) {
    ...
    assert.match(source, /PageHeader/)
    assert.match(source, /helpText=|help-text=/)
    assert.match(source, /data-template-page-header-host="filter-list"/)
    assert.doesNotMatch(source, /data-template-action-host="filter-list"|backable|backable=/)
    assert.match(source, /FilterBar/)
    assert.match(source, /TableToolbar/)
    assert.match(source, /submitMode="manual"|submit-mode="manual"/)
