...
test('the production site keeps deployment overrides and the AI collaboration overview route', async () => {
  ...
  assert.match(overview, /业务内容可以变化，已批准的页面结构默认保持一致/)
  assert.match(overview, /Request[\s\S]*Rules \/ Capability[\s\S]*Template[\s\S]*Shell[\s\S]*Approved Reference[\s\S]*Business Slots[\s\S]*Implementation[\s\S]*Validation/)
...
test('the AI collaboration information architecture exposes all guide and Starter management pages', async () => {
  ...
  assert.doesNotMatch(config, /label:\s*['"](?:V1|V1-r1|V1-r2|V1-r3|启动包版本)['"]/)
  assert.match(usage, /^title:\s*AI 使用指南$/m)
  assert.match(usage, /^## 使用流程$/m)
  assert.match(usage, /用户业务需求[\s\S]*Starter 规则与能力路由[\s\S]*Template Decision[\s\S]*Shell Decision[\s\S]*Approved Reference Binding[\s\S]*业务内容替换[\s\S]*Implementation[\s\S]*Validation/)
  assert.match(usage, /`default`[\s\S]*`content-only`[\s\S]*`none`/)
  assert.match(usage, /Level 1[\s\S]*Level 2[\s\S]*Depth ≥ 3/)
  assert.match(selection, /^title:\s*如何选择启动包$/m)
  assert.match(selection, /V1-r3 当前生产版/)
  assert.match(starters, /^title:\s*启动包总览$/m)
  assert.match(starters, /产品经理启动包[\s\S]*Available \/ V1-r3[\s\S]*前端开发启动包[\s\S]*Coming Soon/)
  assert.match(starters, /\.\.\/\.\.\/downloads\/designkit-starter-v1-r3\.zip/)
...
test('the product manager Starter page exposes capability boundaries, comparison, and traceable releases', async () => {
  ...
  assert.match(releasePage, /^## 当前版本状态$/m)
  ...
  assert.match(releasePage, /^\| 标准应用框架 \|/m)
  assert.match(releasePage, /^\| Shell 模式 \|/m)
  assert.match(releasePage, /^\| 页面模板 \|/m)
  assert.match(releasePage, /^\| 页面层级 \|/m)
  assert.match(releasePage, /^\| AI 生成与验证 \|/m)
  assert.match(releasePage, /批准模板是标准答案/)
  assert.match(releasePage, /\| 本版重点 \|/)
  assert.doesNotMatch(releasePage, /\| 更新内容 \|/)
  assert.match(releasePage, /^### DesignKit Starter V1-r3$/m)
  ...
  assert.match(releasePage, /href="\.\.\/\.\.\/downloads\/designkit-starter-v1-r3\.zip" download="designkit-starter-v1-r3\.zip"/)
  assert.match(releasePage, /更新时间：2026-09-01/)
  assert.match(releasePage, /DesignKit Starter V1-r3<\/h3><span class="sb-starter-current-version__new">NEW<\/span>/)
  assert.doesNotMatch(releasePage, /<dl>/)
  assert.match(releasePage, /bfa3af81fc96d82fac9c5dc45eb48183d9454691b2f5ba0c4d0878ab3ffb7d77/)
  ...
  assert.match(releasePage, /Default Application Shell[\s\S]*Light \/ Dark[\s\S]*Basic List[\s\S]*Card List[\s\S]*Grouped \/ Step \/ Drawer Form[\s\S]*完整 IA 深度规则[\s\S]*Package-only fidelity[\s\S]*Integrated multi-page validation/)
  assert.match(releasePage, /自定义 Application Shell[\s\S]*Navigation API[\s\S]*Router[\s\S]*权限路由[\s\S]*后端动态菜单[\s\S]*React\/Vue 工程[\s\S]*真实 Export[\s\S]*真实后端\/API/)
  assert.match(releasePage, /V1-r2 建立了固定 Default Application Shell 与全局 Light \/ Dark 基线[\s\S]*这些 R3 能力没有回写到 R2 历史描述中/)
  ...
  assert.doesNotMatch(releasePage, /\/private\/tmp\//)
