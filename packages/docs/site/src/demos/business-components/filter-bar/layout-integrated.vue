<template>
  <div class="sb-filter-bar-demo sb-filter-bar-demo--layout sb-filter-bar-demo--integrated">
    <div class="sb-filter-bar-demo__filter-module">
      <FilterBar
        :fields="fields"
    ...
    <div class="sb-filter-bar-demo__result-module">
...
// FILTER_BAR_LAYOUT_SCHEMA_START
const fields: FilterFieldSchema[] = [
  { type: 'input', name: 'keyword', label: '关键词', placeholder: '门店 / 编号', priority: 0 },
  {
    type: 'select',
    name: 'status',
    label: '状态',
  ...
  },
  {
    ...
    priority: 5,
    options: [
      { label: '标准门店', value: 'standard' },
      { label: '臻选门店', value: 'reserve' },
      { label: '啡快门店', value: 'now' },
    ],
  },
  {
    type: 'select',
  ...
  { type: 'input', name: 'owner', label: '负责人', placeholder: '姓名 / 工号', priority: 7 },
  { type: 'dateRange', name: 'period', label: '营业周期', priority: 8 },
];

const defaultValue: FilterValue = { status: 'open', region: ['east', 'shanghai'] };
const layoutColumns = { xs: 1, sm: 3, md: 3, lg: 3, xl: 3, xxl: 3 };
...
// FILTER_BAR_LAYOUT_SCHEMA_END

const activeValues = ref<FilterValue>({ ...defaultValue });
const visibleRows = computed(() => rows.filter((row) => !activeValues.value.status || row.status === activeValues.value.status));
